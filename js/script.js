...
const pilihan = document.querySelectorAll("#play-area li");
let pScore = 0;
let cScore = 0;
let playing;
...
pilihan.forEach(x => {
  x.addEventListener("click", () => {
    ...
    const compImg = document.querySelector("#comp-img");
		...
		pilihan.forEach(y => {
			if (y == x) {
				y.style.transform = "scale(1.03) translateY(-10px)";
				y.style.boxShadow = "0 30px 15px -10px #555";
				return;
			}
    ...
    setTimeout(() => {
      ...
      pilihan.forEach(x => x.attributeStyleMap.clear());
